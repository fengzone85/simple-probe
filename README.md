...
## 关于 CF VPS Monitor 的澄清（源码级证据）

本项目之外常被拿来对比的 CF VPS Monitor（`kadidalax/cf-vps-monitor`）同样采用「服务端→Agent 指令通道」架构。针对网上两种常见误判，这里用其 `agent/main.go` 的真实源码给出明确结论。
...
> 一句话：CF VPS Monitor 是「受限的受控探测代理」，不是「RCE 后门」。它的问题不在能 RCE，而在「指令通道存在本身」——这正是本项目刻意不做的事。
