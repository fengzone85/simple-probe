...
6. **速率限制** —— Nginx 对上报接口限流（基于真实 TCP 对端，是真实安全边界）。服务端另有应用层兜底限流（每 IP 每 10s 20 次，注册/初始化各有限流），但其 `req.ip` 取自反代注入的 `X-Forwarded-For`，**攻击者可伪造**，故应用层限流**不是安全边界**、仅作兜底；生产必须以 Nginx `limit_req` 为准，且严禁将 8080 端口直接暴露公网。
...
11. **Dashboard 两步验证（TOTP）** —— 可在「安全」面板启用。启用后，所有**管理写操作**（建/改/删客户端、重置 Token、测试告警）除静态 Token 外还需动态码，纯静态 Admin Token 单独无法执行写操作；前端登录改由签名 `HttpOnly + Secure + SameSite=Strict` Cookie 维持，**不再在前端明文存储 Token**（消除 XSS 窃取风险）。只读拉取（Grafana、`/metrics`，以 `ADMIN_TOKEN` 经 Bearer 鉴权，且除 `ADMIN_ALLOW_HTTP=1` 外强制 HTTPS）保持无感、不强制 2FA。详见下文「两步验证（TOTP）」。
...
- 只读监控（Grafana 拉取、`/metrics`，以 `ADMIN_TOKEN` Bearer 鉴权）不受影响，保持无感。
...
> 结论：谛听轻量探针 用功能上的「减法」（去指令通道、去指纹、去互感知）换取安全上的「加法」。攻击者无法利用不存在的东西。
...
| 谛听轻量探针 | Node + SQLite / Python | 纯监控（无控制） | 无 | 原生 ~15–25 MB | 早期 |

### 功能与安全的取舍

- **是否需要「控制」**：哪吒的 Web 终端、Komari 的远程控制，能让你直接在面板上操作被控机，便利，但需要被控端保留指令通道与执行权限。谛听轻量探针 不提供任何控制能力，被控端只出站、不接收指令——这是本项目刻意的选择，详见上文「为什么我们刻意不实现这些功能」与「威胁模型：信任边界分析」。
- **是否需要更丰富的监控类型**：哪吒额外支持 SSL 证书、TCP、网站监控；谛听轻量探针 当前聚焦系统指标（CPU / 内存 / 磁盘 / 负载 / 网络 / 流量 / 温度 / Swap / 开机时长 / 网络质量自测）。
- **隐私取向**：哪吒依赖 GeoIP（采集 IP / 地理位置），Pulse 采集 IP / 位置 / CPU 型号；谛听轻量探针 不采集任何可用于指纹识别的数据。
- **资源取向**：对 256–512 MB 的小内存机器，谛听轻量探针 的原生 systemd 形态（约 15–25 MB）比 Docker 形态（含守护进程 65–150 MB）更省，也与 Go 二进制量级接近。
...
- 只需查看机器状态、在意资源占用 / 隐私 / 不想被控：**谛听轻量探针**更合适。

> 说明：以上仅作中性参考。各项目均在活跃维护，具体能力以各项目最新版本为准；谛听轻量探针 的取舍逻辑见「设计原则」与「威胁模型」两节。
...
| P2 | 加固项（④⑤⑥⑦） | ✅ | 早期会话已修复 |
| P3 ⑩ | Prometheus `/metrics` | ✅ | `GET /metrics` 暴露 `cpu/mem/disk/net` 等指标，仅 `ADMIN_TOKEN` Bearer 鉴权（除 `ADMIN_ALLOW_HTTP=1` 外强制 HTTPS；`READONLY_TOKEN` 不适用） |
